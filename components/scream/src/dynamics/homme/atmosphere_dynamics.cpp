...
void HommeDynamics::set_grids (const std::shared_ptr<const GridsManager> grids_manager)
{
  ...
  //  say, having NTL separate array) because of memory locality.
  //  At the end of Homme's timestep, only one of those slices
  //  will be meaningful. When calling set_required|computed_field,
  //  we will set some extra data in the field, storing the current
  //  value for the "correct" slice. This info will be used by the
  //  phys-dyn remapper to know exactly where to copy data from/to
  //  during the remap.
  ...
  // Inputs
  FieldLayout vector_layout  { {EL,CMP,GP,GP,VL}, {ne,nmf,NGP,NGP,NVL} };
  FieldLayout scalar_layout  { {EL,    GP,GP,VL}, {ne,    NGP,NGP,NVL} };
  ...
  // Input/Outputs: states are 'updated' by homme, so they are both in and out fields.
  ...
  m_required_fields.emplace("u",       dyn_vector_3d_mid_layout, m/s,            dyn_grid_name);
  m_required_fields.emplace("vtheta",  dyn_scalar_3d_mid_layout, K,              dyn_grid_name);
  m_required_fields.emplace("phi",     dyn_scalar_3d_int_layout, Pa*pow(m,3)/kg, dyn_grid_name);
  m_required_fields.emplace("w",       dyn_scalar_3d_int_layout, m/s,            dyn_grid_name);
