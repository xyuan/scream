...
!ASD - Ignore For Now    call pbuf_get_field(pbuf, cmeliq_idx,      cmeliq                                                )
!ASD - Ignore For Now    ! OUTPUTS
!ASD - Ignore For Now    call pbuf_get_field(pbuf,    prec_str_idx,  prec_str)
!ASD - Ignore For Now    call pbuf_get_field(pbuf,    snow_str_idx,  snow_str)
!ASD - Ignore For Now    call pbuf_get_field(pbuf,    prec_sed_idx,  prec_sed)
!ASD - Ignore For Now    call pbuf_get_field(pbuf,    snow_sed_idx,  snow_sed)
!ASD - Ignore For Now    call pbuf_get_field(pbuf,    prec_pcw_idx,  prec_pcw)
!ASD - Ignore For Now    call pbuf_get_field(pbuf,    snow_pcw_idx,  snow_pcw)
!ASD - Ignore For Now    !============================ 
!ASD - Ignore For Now    ! All internal PBUF variables
!ASD - Ignore For Now    ! INPUTS
!ASD - Ignore For Now    call pbuf_get_field(pbuf,      relvar_idx,    relvar                                                   )
!ASD - Ignore For Now    ! OUTPUTS
!ASD - Ignore For Now    call pbuf_get_field(pbuf,        cldo_idx,      cldo, start=(/1,1,itim_old/), kount=(/psetcols,pver,1/))
!ASD - Ignore For Now    call pbuf_get_field(pbuf,         qme_idx,       qme                                                   )
!ASD - Ignore For Now    call pbuf_get_field(pbuf,       precip_total_tend_idx,     precip_total_tend                                                   )
!ASD - Ignore For Now    call pbuf_get_field(pbuf,      nevapr_idx,    nevapr                                                   )
!ASD - Ignore For Now    call pbuf_get_field(pbuf,   qr_evap_tend_idx, qr_evap_tend                                                   )
!ASD - Ignore For Now    call pbuf_get_field(pbuf,         rei_idx,       rei                                                   ) ! ice eff. rad
!ASD - Ignore For Now    call pbuf_get_field(pbuf,         rel_idx,       rel                                                   ) ! liq. eff. rad
!ASD - Ignore For Now    call pbuf_get_field(pbuf,         dei_idx,       dei                                                   )
!ASD - Ignore For Now    call pbuf_get_field(pbuf,          mu_idx,        mu                                                   )
!ASD - Ignore For Now    call pbuf_get_field(pbuf,     lambdac_idx,   lambdac                                                   )
!ASD - Ignore For Now    call pbuf_get_field(pbuf,   ls_flxprc_idx,    flxprc                                                   )
!ASD - Ignore For Now    call pbuf_get_field(pbuf,   ls_flxsnw_idx,    flxsnw                                                   )
!ASD - Ignore For Now    call pbuf_get_field(pbuf, ls_reffrain_idx,  reffrain                                                   ) 
!ASD - Ignore For Now    call pbuf_get_field(pbuf, ls_reffsnow_idx,  reffsnow                                                   ) 
!ASD - Ignore For Now    call pbuf_get_field(pbuf,  cv_reffliq_idx, cvreffliq                                                   )
!ASD - Ignore For Now    call pbuf_get_field(pbuf,  cv_reffice_idx, cvreffice                                                   )
    ...
    !==============
!ASD - Ignore For Now    cldo(:ncol,top_lev:pver)=ast(:ncol,top_lev:pver)
    
!ASD - Ignore For Now    ! INITIALIZE PTEND
!ASD - Ignore For Now    !==============
!ASD - Ignore For Now    !ptend is an output variable. Since not substepping in micro, don't need 
!ASD - Ignore For Now    !a local copy.
!ASD - Ignore For Now    lq            = .false. !initialize all constituents to false.
!ASD - Ignore For Now    lq(1)         = .true.
!ASD - Ignore For Now    lq(ixcldliq)  = .true.
!ASD - Ignore For Now    lq(ixcldice)  = .true.
!ASD - Ignore For Now    lq(ixnumliq)  = .true.
!ASD - Ignore For Now    lq(ixnumice)  = .true.
!ASD - Ignore For Now    lq(ixrain)    = .true.
!ASD - Ignore For Now    lq(ixcldrim)  = .true.
!ASD - Ignore For Now    lq(ixnumrain) = .true.
!ASD - Ignore For Now    lq(ixrimvol)  = .true.
!ASD - Ignore For Now    call physics_ptend_init(ptend, psetcols, "micro_p3", ls=.true., lq=lq)
