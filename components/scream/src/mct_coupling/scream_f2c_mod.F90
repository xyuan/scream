...
interface
  ...
  subroutine scream_setup_surface_coupling (x2a_names, x2a_indices, x2a_ptr, VecComp_x2a, &
                                            num_cpl_imports, num_scream_imports, &
                                            a2x_names, a2x_indices, a2x_ptr, VecComp_a2x, &
                                            num_exports) bind(c)
    use iso_c_binding, only: c_ptr, c_int
    !
    ! Input(s)
    !
    type(c_ptr),         intent(in) :: x2a_indices, x2a_names, x2a_ptr, VecComp_x2a
    type(c_ptr),         intent(in) :: a2x_indices, a2x_names, a2x_ptr, VecComp_a2x
